...
**Fechas de Ejecución:** 23/05 al 28/05/2026  
...
## 🚀 SETUP INICIAL - Hacer el 23/05 (Sábado)
...
- En **Panel Admin ➡️ Clientes ➡️ [Editar]**:
  - **Nombre:** Pablo
  - **Fecha de Nacimiento:** **25/05/1993** (Configurada para el **25/05**, clave para la prueba del Día 3).
...
- **Nombre:** Simba
- **Marca:** Purina
- **Ciclo de alimentación:** **7 días** (Crítico para que el aviso de alimento se dispare el **27/05**).
  > 💡 *Compra el 23/05 + ciclo 7 días = Agotamiento el 30/05. Restando 3 días de aviso = Alerta el 27/05.*
...
- **Fecha:** **24/05/2026** (Domingo)
- **Hora inicio:** **00:01**
- **Hora fin:** **23:59**
- **Días habilitados:** Domingo
...
- **Fecha inicio:** **28/05/2026**
- **Fecha fin:** **29/05/2026**
- **Estado:** Activo
...
### 📅 DÍA 1 - 23/05 (Sábado)   Simulador Offset: 0
...
- **Monto:** **200 puntos** (Esto define 5 días de validez ➡️ vencen el 28/05).
- **Reponer Alimento:** ✅ Marcar y seleccionar la mascota (Simba).
- **Concepto:** `Compra QA`
- *Verifica en el perfil que figure: "Próximo vencimiento: 28/05/2026 con 200 pts".*
...
- **Silencio de Vencimiento de Bienvenida:** No debe generarse ninguna alerta de vencimiento en el panel ni enviarse notificaciones de expiración. 
  > 💡 *Esto es correcto porque hoy es 23/05 y faltan exactamente 5 días para el vencimiento (28/05). Al configurar `expirationWarningDays = 4`, sumamos el día de espera solicitado para evitar el bucle o embudo inmediato de alertas al registrar al usuario.*

---

### 📅 DÍA 2 - 24/05 (Domingo)   Simulador Offset: +1
**Objetivo:** Validar la Campaña Flash, la Tolerancia Estricta de 0 minutos y el **Primer Aviso de Vencimiento**.

#### 1. Cambio de Fecha:
En **Simulador ➡️ [+1 día]** (La fecha del panel pasa al 24/05).
...
- **Primer Aviso de Vencimiento (Parchado a 4 días):**
  ...
  - **Push / Inbox PWA:** "⚠️ Tus puntos vencen pronto ⏳ 200 pts el 28/05".
  - **Base de Datos (Firestore):** El campo `lastExpirationWarningDates` en el documento del usuario se actualiza a `{"2026-05-28": "2026-05-24"}`.
  > 💡 *Se activa exitosamente en el Día 2 porque hoy es 24/05 y faltan exactamente 4 días para el vencimiento (28/05).*
- **Campaña Flash:**
...
### 📅 DÍA 3 - 25/05 (Lunes)   Simulador Offset: +2
**Objetivo:** Validar saludo de cumpleaños automático y puntos de regalo.

#### 1. Cambio de Fecha:
En **Simulador ➡️ [+1 día]** (La fecha pasa al 25/05).
...
### 📅 DÍA 4 - 26/05 (Martes)   Simulador Offset: +3
...
- El día transcurre sin avisos repetidos de vencimiento.
  > 💡 *No se dispara el segundo aviso de vencimiento porque han transcurrido sólo 2 días desde el primer aviso (24/05 al 26/05) y el intervalo es de 3 días.*

---

### 📅 DÍA 5 - 27/05 (Miércoles)   Simulador Offset: +4
...
- **Segundo Aviso de Vencimiento (Intervalo de 3 días transcurridos):**
  ...
  - **Push / Inbox PWA:** Segundo recordatorio de vencimiento para el 28/05.
  - **Base de Datos (Firestore):** El campo `lastExpirationWarningDates` en el documento del usuario se actualiza a `{"2026-05-28": "2026-05-27"}`.
  > 💡 *Se dispara porque han pasado exactamente 3 días desde el primer aviso (24/05 al 27/05).*
- **Alerta Predictiva de Alimento:**
  ...
  > 💡 *Se calcula: compra 23/05 + ciclo 7 días = fin 30/05 - 3 días de anticipación = 27/05.*

---

### 📅 DÍA 6 - 28/05 (Jueves)   Simulador Offset: +5
...
- **Backend:** Los 200 puntos cargados el día 23 expiran automáticamente al final del día.
