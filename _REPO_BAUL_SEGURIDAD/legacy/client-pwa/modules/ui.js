...
export function renderMainScreen(clienteData, premiosData, campanasData = [], opts = {}) {
  ...
  // Saldo "a favor" (remanente de puntos)
  const saldoCard = document.getElementById('saldo-card');
  if (saldoCard) {
    const rawSaldo = Number(clienteData.saldoFavor || 0);
    const s = Math.floor(rawSaldo); // Sin decimales
    if (s > 0) {
      safeSetText('cliente-saldo', `$ ${s}`);

      // Calcular falta
      const pointsRatio = Number(window.APP_CONFIG?.pointsPerPeso || 1);
      const moneyBase = Number(window.APP_CONFIG?.pointsMoneyBase || 100);
      const costPerPoint = moneyBase / pointsRatio;
      const missing = Math.ceil(costPerPoint - (rawSaldo % costPerPoint));

      const infoEl = saldoCard.querySelector('.info-message');
      if (infoEl) {
        infoEl.innerHTML = `A favor para sumar puntos. <br>Te faltan <strong style="color:#e91e63; font-weight:700;">$ ${missing}</strong> para el próximo punto.`;
      }
