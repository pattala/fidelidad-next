...
$stateCode = @"
    ...
    const openTestModal = (key: string, customTitle?: string) => {
        const t = config.messaging?.templates?.[key] || '';
        const title = config.messaging?.templates?.[key + '_title'] || customTitle || '';
        const wa = config.messaging?.templates?.[key + '_whatsapp'] || '';
        const ch = config.messaging?.eventConfigs?.[key]?.channels || [];
        setTestMessageModal({
            isOpen: true,
            templateKey: key,
            title: title,
...
$content = $content -replace 'const \[waPreview, setWaPreview\] = useState\(\{', "$stateCode`n`n    const [waPreview, setWaPreview] = useState({"
                    ...
                    <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="relative">
                                    ...
                                    <input 
                                        ...
                                        onChange={async (e) => {
                                            ...
                                            window._testUsers = results; // Guardar temporalmente
                                ...
                                <div id="selectedTestUser" className="hidden bg-green-50 border border-green-200 rounded-xl p-4 flex items-center justify-between">
                                    ...
                                    <button onClick={() => {
                                        ...
                                        window._selectedUserForTest = null;
                        ...
                        <div className="p-6 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
                            ...
                            <button onClick={async () => {
                                const user = window._selectedUserForTest;
...
# Find all occurrences of the Email preview button and insert the Test button next to them
$emailPreviewBtn = " title=`"Vista Previa Email`"><Eye size={16} /></button>"
$testBtn = " title=`"Vista Previa Email`"><Eye size={16} /></button>`n                                                            <button type=`"button`" onClick={() => openTestModal('__TEMPLATE__')} className=`"px-2 py-1.5 text-purple-600 hover:text-purple-800 rounded hover:bg-purple-50 transition border border-purple-200`" title=`"Probar Envío a Usuario`"><Rocket size={16} /></button>"
...
# Referral Challenge (has custom title in openPreview)
...
# Pet Food Alert (Doesn't have an openPreview button, we inject next to the restore button)
$restoreBtn = " title=`"Restaurar`">↺</button>"
$testBtnPet = " title=`"Restaurar`">↺</button>`n                                                            <button type=`"button`" onClick={() => openPreview('petFoodAlert')} className=`"px-2 py-1.5 text-blue-500 hover:text-blue-700 rounded hover:bg-blue-50 transition border border-blue-100`" title=`"Vista Previa Email`"><Eye size={16} /></button>`n                                                            <button type=`"button`" onClick={() => openTestModal('petFoodAlert')} className=`"px-2 py-1.5 text-purple-600 hover:text-purple-800 rounded hover:bg-purple-50 transition border border-purple-200`" title=`"Probar Envío a Usuario`"><Rocket size={16} /></button>"
$content = $content -replace "onClick=\{\(\) => setConfig\(\{ \.\.\.config, messaging: \{ \.\.\.config\.messaging!, templates: \{ \.\.\.config\.messaging\?\.templates, petFoodAlert: DEFAULT_TEMPLATES\.petFoodAlert \} \} \}\)\} className=`"px-2 py-1\.5 text-gray-400 hover:text-orange-600 rounded hover:bg-orange-50 transition text-sm`" title=`"Restaurar`">↺</button>", "onClick={() => setConfig({ ...config, messaging: { ...config.messaging!, templates: { ...config.messaging?.templates, petFoodAlert: DEFAULT_TEMPLATES.petFoodAlert } } })} className=`"px-2 py-1.5 text-gray-400 hover:text-orange-600 rounded hover:bg-orange-50 transition text-sm`"$testBtnPet"
