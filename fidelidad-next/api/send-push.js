        ...
        if (!dryRun) {
          try {
            const resp = await fcm.sendEachForMulticast({
              tokens: r.tokens,
              notification: { title: titulo, body: cuerpo }, // ✅ Restore for System/Mobile support
              data: {
                templateId,
                uid: r.uid,
...
async function pruneInvalidTokens(db, tokens) {
  ...
  const snap = await db.collection("users").where('fcmTokens', 'array-contains-any', slice).get();
