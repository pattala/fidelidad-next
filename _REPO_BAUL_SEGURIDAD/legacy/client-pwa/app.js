...
// ──────────────────────────────────────────────────────────────
// Redes Sociales (Dynamic Content)
// ──────────────────────────────────────────────────────────────
async function setupSocialLinks() {
  ...
  try {
    let contact = window.APP_CONFIG?.contact;
    ...
    try {
      ...
      if (snap.exists) {
        ...
        // Merge into window.APP_CONFIG for global access
        window.APP_CONFIG = { ...window.APP_CONFIG, ...dbConfig };
        contact = dbConfig.contact || contact;
      }
    } catch (e) {
      console.warn('Global config fetch fail', e);
    }

    if (!contact) return;

    const links = [];
    const iconStyle = "width:32px; height:32px; filter: drop-shadow(0 1px 2px rgba(0,0,0,0.1)); display:block;";

    // No margins inside links, gap handled by CSS .bottom-nav

    if (contact.whatsapp) {
      const num = contact.whatsapp.replace(/\D/g, '');
      links.push(`<a href="https://wa.me/${num}" target="_blank" style="display:flex;align-items:center;justify-content:center;"><img src="https://cdn.simpleicons.org/whatsapp" alt="WhatsApp" style="${iconStyle}" /></a>`);
    }
    if (contact.instagram) {
      const url = contact.instagram.startsWith('http') ? contact.instagram : `https://instagram.com/${contact.instagram.replace('@', '')}`;
      links.push(`<a href="${url}" target="_blank" style="display:flex;align-items:center;justify-content:center;"><img src="https://cdn.simpleicons.org/instagram" alt="Instagram" style="${iconStyle}" /></a>`);
    }
    if (contact.facebook) {
      links.push(`<a href="${contact.facebook}" target="_blank" style="display:flex;align-items:center;justify-content:center;"><img src="https://cdn.simpleicons.org/facebook" alt="Facebook" style="${iconStyle}" /></a>`);
    }
    if (contact.website) {
      links.push(`<a href="${contact.website}" target="_blank" style="text-decoration:none; font-size:24px;" title="Web">🌐</a>`);
    }
    if (contact.email) {
      links.push(`<a href="mailto:${contact.email}" style="text-decoration:none; font-size:26px;" title="Email">✉️</a>`);
    }
    ...
    console.warn('[Social] Error rendering links:', err);
  }
}
  ...
  auth.onAuthStateChanged(async (user) => {
    ...
    if (user) {
      ...
      if (justSignedUp) {
        ...
        UI.showScreen('main-app-screen');
        setupMainAppScreenListeners(); // Initialize logic for Main Screen
...
// ──────────────────────────────────────────────────────────────
// CARRUSEL DE CAMPAÑAS (Anuncios y Promos)
// ──────────────────────────────────────────────────────────────
let carruselInterval = null;

async function initCarousel() {
  const container = document.getElementById('carrusel-campanas-container');
  const track = document.getElementById('carrusel-campanas');
  const indicators = document.getElementById('carrusel-indicadores');

  if (!container || !track) return;

  try {
    const today = new Date();
    // Normalizar "hoy" a medianoche para comparaciones de fecha seguras si se necesita
    // today.setHours(0,0,0,0);
    const todayStr = today.toISOString().split('T')[0]; // YYYY-MM-DD
    const dayOfWeek = today.getDay(); // 0=Dom

    // Query básica: activas y que se muestren en App
    const snapshot = await db.collection('bonus_rules')
      .where('active', '==', true)
    ...
    snapshot.forEach(doc => {
      ...
      // 1. Filtro Dia Semana
      if (data.daysOfWeek && !data.daysOfWeek.includes(dayOfWeek)) return;

      // 2. Filtro Fechas (Active Range)
      if (data.startDate && data.startDate > todayStr) return;
      if (data.endDate && data.endDate < todayStr) return;

      // 3. Filtro Imagen (Es obligatorio para el carrusel, aunque sea un placeholder)
    ...
    campaigns.forEach((camp, index) => {
      ...
      if (camp.description) {
        ...
        cap.className = 'carrusel-caption';
        cap.textContent = camp.description;
        slide.appendChild(cap);
      } else if (camp.name) {
        // Fallback title (especialmente para lectores de pantalla)
        img.title = camp.name;
      }
    ...
    if (campaigns.length > 1) {
      ...
      // Limpiar previo si hubo re-render
      if (carruselInterval) clearInterval(carruselInterval);
      carruselInterval = setInterval(nextSlide, 5000); // 5 seg

      // Touch events para swipe simple (opcional pero bueno para UX móvil)
      // TODO: swipe logic
    }

  } catch (error) {
    console.warn('[Carrusel] Error cargando campañas:', error);
    container.style.display = 'none';
  }
}
...
// Inyección de Estilos para el Carrusel
const carouselStyles = `
  ...
  .dot.active {
    background: #6f42c1; /* Purple match brand */
    transform: scale(1.2);
  }
`;
...
// MODIFICADO: Agregar llamada en setupMainAppScreenListeners
// (Duplicate removed)
