                        ...
                        <div className="max-w-3xl mx-auto space-y-8 animate-fade-in">
                            ...
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
                                ...
                                <div className="space-y-6">
                                    ...
                                    {/* 3. PWA INSTALLATION MOMENTS (Celebration) */}
                                    <div className="bg-emerald-50/30 p-6 rounded-2xl border border-emerald-100/50 space-y-4">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <h4 className="font-black text-gray-800 uppercase tracking-tighter">Instalación PWA (Momentos de Gloria)</h4>
                                                    <p className="text-xs text-gray-500">Cartel festivo que aparece cuando el socio gana puntos.</p>
                                                </div>
                                            </div>
                                            <button
                                                type="button"
                                                onClick={() => setConfig({
                                                    ...
                                                    messaging: {
                                                        ...config.messaging!,
                                                        enablePwaInstallPromptRepetition: !config.messaging?.enablePwaInstallPromptRepetition
                                                    }
                                                })}
                                                className={`relative w-12 h-7 transition-colors rounded-full ${config.messaging?.enablePwaInstallPromptRepetition ? 'bg-emerald-600' : 'bg-gray-200'}`}
                                            >
                                                <span className={`absolute top-1 left-1 bg-white w-5 h-5 rounded-full shadow-sm transition-transform ${config.messaging?.enablePwaInstallPromptRepetition ? 'translate-x-5' : 'translate-x-0'}`} />
                                            </button>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div className="bg-white p-4 rounded-xl border border-emerald-100 shadow-sm space-y-3">
                                                <div className="flex justify-between items-center">
                                                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Ciclo de Intentos</label>
                                                    <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded">PASO 1</span>
                                                </div>
                                                <div className="flex gap-4">
                                                    <div className="flex-1">
                                                        <label className="block text-[10px] font-bold text-gray-400 mb-1">Máx. Intentos</label>
                                                        <div className="relative">
                                                            <input
                                                                type="number" min="1" max="10"
                                                                value={config.messaging?.pwaInstallPromptMaxAttempts || 3}
                                                                onChange={e => setConfig({ ...config, messaging: { ...config.messaging!, pwaInstallPromptMaxAttempts: parseInt(e.target.value) || 3 } })}
                                            ...
                                            <div className={`p-4 rounded-xl border transition-all shadow-sm space-y-3 ${config.messaging?.enablePwaInstallPromptRepetition ? 'bg-white border-emerald-100' : 'bg-gray-50 border-gray-200 grayscale opacity-75'}`}>
                                                ...
                                                <p className="text-[9px] text-gray-400 leading-tight">
                                                    {config.messaging?.enablePwaInstallPromptRepetition 
                                                        ? `Después de agotarse los ${config.messaging?.pwaInstallPromptMaxAttempts || 3} intentos, esperaremos ${config.messaging?.pwaInstallPromptResetDays || 30} días para volver a reiniciar.`
